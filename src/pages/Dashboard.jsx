    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center gap-3 pt-1">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-footnote text-gray-400 dark:text-gray-500 font-medium">{formatDate(today)}</p>
          <h1 className="text-title-2 text-gray-900 dark:text-white truncate">
      ...
      <div className="flex flex-col gap-3">
        <p className="text-footnote font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide px-0.5">Today</p>
        <div className="flex flex-col gap-2.5">
          {quickActions.map(({ to, emoji, label, sub, color }) => (
            <Link key={to} to={to}>
              <div className="bg-white dark:bg-[#1C1C1E] rounded-apple-lg shadow-apple-card dark:shadow-none px-4 py-3.5 flex items-center gap-4 active:scale-[0.98] transition-transform">
                <div className={`${color} w-10 h-10 rounded-apple flex items-center justify-center flex-shrink-0`}>
                  <span className="text-xl">{emoji}</span>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-headline text-gray-900 dark:text-white font-semibold">{label}</p>
                  <p className="text-footnote text-gray-400 dark:text-gray-500 mt-0.5">{sub}</p>
                </div>
      ...
      {mealPlan && (
        <Card className="p-4">
          <p className="text-footnote font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-2">Today's meals</p>
          <Link to="/diet" className="text-callout text-brand-600 dark:text-brand-400 font-medium">
            View meal plan →
          </Link>
        </Card>
      )}
      ...
      {profile?.is_coach && members.length > 0 && (
        <div className="flex flex-col gap-2">
          <p className="text-footnote font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide px-0.5">Your members</p>
          <Card className="divide-y divide-[#F2F2F7] dark:divide-white/10 overflow-hidden">
            {members.map(m => (
              <Link
                key={m.user_id}
                to={`/profiles/${m.user_id}`}
                className="flex items-center gap-3 px-4 py-3 active:bg-[#F2F2F7] dark:active:bg-white/10 transition-colors"
              >
                <Avatar src={m.avatar_url} name={m.display_name} size="sm" />
                <span className="flex-1 text-callout font-medium text-gray-900 dark:text-white">{m.display_name}</span>
